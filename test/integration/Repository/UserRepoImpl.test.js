...
import UserRepoImpl from '../../../src/infrastructure/repository/UserRepoImpl';
...
test('Initialise Sequelize', () => {
    try {
        ...
        expect(sequelizeRepo).toBeDefined();
    }
    catch (e) {
        console.dir(e);
        throw e;
...
test('Create User', async () => {
    try {
        const sequelizeDatabaseObject = new SequelizeDatabaseObject();
        const sequelizeRepo = new UserRepoImpl(sequelizeDatabaseObject);
        ...
        const createUser = await sequelizeRepo.createUser(user);
        const receivedUser = await sequelizeRepo.getUserById(id);
